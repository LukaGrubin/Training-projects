...
def is_valid(en):
    for k in range(len(en)):
        if en[k] in 'АБВБДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЬЫЭЮЯ':
            return True
        else:
            return False
...
def play(word):
    ...
    print('Угодай слово:', *word_completion,sep = '')

    while guessed == False:

        print("Введите букву или слово")
        key_word = input().upper()

        if is_valid(key_word) == True:
            key_word = key_word
        else:
            print('Для ввода доступны только русские буквы!!!')

        if len(key_word) == 1 and key_word in word:

            if key_word in word_completion:
                print('Эту букву вы уже угадывали, попробуйте другую')

            for i in range(len(word)):
                if key_word == word[i]:
                    word_completion[i] = key_word

            print(*word_completion, sep='')

        if key_word == word or word_completion == list(word):
            print('Вы угадали! '
                  'Поздравляю с победой!')
            break

        if key_word not in word and is_valid(key_word) == True:
            if key_word in guessed_letters or key_word in guessed_words:
                print('Это вы уже назыввли!')
            else:
                tries -= 1
                if len(key_word) == 1:
                    guessed_letters.append(key_word)
                else:
                    guessed_words.append(key_word)
